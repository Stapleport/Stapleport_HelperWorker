...
> 费率口径：ImputePay 当前**零平台抽成**（helper 净得换币后全额）；合约已预留按奖励比例抽成机制（`fee_bps`，上限 50%，激活 = MANAGE 一笔 `setFee` 交易）——激活后本 Worker 盈利预检自动按净份额折算，无需改代码。
...
**双仓发布制**：本仓是正本（接 kit 正典，monorepo 内）；对外自托管分发走公开发布仓 [Stapleport/HelperWorker_Release](https://github.com/Stapleport/HelperWorker_Release)（public，pay-kit/worker-kit 源码 vendored、Deploy to Cloudflare 一键部署；⚠️ 该仓是发布产物，勿在彼处开发，一律回本仓改）。kit 或本仓变更后重跑发布：
